...
def get_data_loaders(args, batch_size=64):
    num_samples = 1000
    data_s = torch.randn(num_samples, args.dimensions)
    data_f = torch.randn(num_samples, args.dimensions)
    idx = torch.arange(num_samples)
    labels = torch.randint(0, 2, (num_samples, 1)).float()

    dataset = TensorDataset(data_s, data_f, idx, labels)
...
def consistency_loss(z1, z2):
    ...
    # Cosine similarity loss between original and perturbed node representations.
