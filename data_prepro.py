...
def load_data(args, n_splits=5):
    ...
    positive = np.concatenate(
        [positive, np.ones(positive.shape[0], dtype=np.int64).reshape(-1, 1)], axis=1)
    negative = np.concatenate(
        [negative, np.zeros(negative.shape[0], dtype=np.int64).reshape(-1, 1)], axis=1)
    ...
    for train_index, test_index in kf.split(all_data):
        ...
        if args.task_type == 'LDA':
            ...
            l_d = l_d.toarray()
            m_d = np.loadtxt("../data/dataset1/mi_dis_association_new2.txt")
            l_m = np.loadtxt("../data/dataset1/lnc_mi_interaction_new2.txt")

            a = 0.9
            b = 0.6
            l_l_t = np.loadtxt(
                "../data/dataset1/one_hot_lnc_sim_" + str(a) + str(b) + ".txt")
            m_m_t = np.loadtxt(
                "../data/dataset1/one_hot_mi_sim_" + str(a) + str(b) + ".txt")
            d_d_t = np.loadtxt(
                "../data/dataset1/one_hot_dis_sim_" + str(a) + str(b) + ".txt")

            l_l_f = np.loadtxt(
                "../data/dataset1/lnc_att_graph.txt")  # 230, 230
            m_m_f = np.loadtxt("../data/dataset1/mi_att_graph.txt")  # 436, 436
            d_d_f = np.loadtxt(
                "../data/dataset1/dis_att_graph.txt")  # 405, 405
        ...
        if args.task_type == 'MDA':
            ...
            # print("adj: ",np.sum(l_d),l_d)
            m_d = m_d.toarray()
            l_d = np.loadtxt("../data/dataset1/lnc_dis_association_new2.txt")
            l_m = np.loadtxt("../data/dataset1/lnc_mi_interaction_new2.txt")

            a = 0.9
            b = 0.5
            l_l_t = np.loadtxt("../data/dataset1/one_hot_lnc_sim_" +
                               str(a) + str(b) + ".txt")
            m_m_t = np.loadtxt("../data/dataset1/one_hot_mi_sim_" +
                               str(a) + str(b) + ".txt")
            d_d_t = np.loadtxt("../data/dataset1/one_hot_dis_sim_" +
                               str(a) + str(b) + ".txt")
        ...
        if args.task_type == 'LMI':
            ...
            b = 0.5
            l_l_t = np.loadtxt("../data/dataset1/one_hot_lnc_sim_" +
                               str(a) + str(b) + ".txt")  # 230, 230
            m_m_t = np.loadtxt("../data/dataset1/one_hot_mi_sim_" +
                               str(a) + str(b) + ".txt")  # 436, 436
            d_d_t = np.loadtxt("../data/dataset1/one_hot_dis_sim_" +
                               str(a) + str(b) + ".txt")  # 405, 405
        ...
        features = np.random.normal(
            loc=0, scale=1, size=(s_adj.shape[0], args.dimensions))
        ...
        # Build data loader
        params = {'batch_size': args.batch, 'shuffle': True,
        ...
        edge_index_s = torch.tensor(
            np.vstack((edges_s[0], edges_s[1])), dtype=torch.long)

        edges_f = f_adj.nonzero()
        edge_index_f = torch.tensor(
            np.vstack((edges_f[0], edges_f[1])), dtype=torch.long)
